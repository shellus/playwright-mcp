...
* ------v------*/
    ...
    initAutoClipboard() {
        if (UI.autoClipboardInitialized) {
            return;
        }
        UI.autoClipboardInitialized = true;

        if (!navigator.clipboard || typeof navigator.clipboard.readText !== 'function') {
            return;
        }

        UI.autoClipboardFocusHandler = async () => {
            if (!UI.rfb) {
                return;
            }
            try {
                const text = await navigator.clipboard.readText();
                if (UI.rfb && typeof text === 'string') {
        ...
        UI.autoClipboardKeydownHandler = (event) => {
            const isPaste = (event.ctrlKey || event.metaKey) &&
                (event.key === 'v' || event.key === 'V');
            if (isPaste) {
                UI.autoClipboardFocusHandler();
            }
        };

        window.addEventListener('focus', UI.autoClipboardFocusHandler, true);
        document.addEventListener('keydown', UI.autoClipboardKeydownHandler);
    },
 ...
 * ------v------*/
    ...
    disconnectFinished(e) {
        ...
        if (UI.autoClipboardFocusHandler) {
            window.removeEventListener('focus', UI.autoClipboardFocusHandler, true);
        }
        if (UI.autoClipboardKeydownHandler) {
            document.removeEventListener('keydown', UI.autoClipboardKeydownHandler);
        }
